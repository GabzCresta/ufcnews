...
export function BottomNav({ eventoId, isAoVivo, username }: BottomNavProps) {
  ...
  const tabs: NavTab[] = [
    ...
    {
      label: t('nav_event'),
      href: eventoId ? `/hub/arena/evento/${eventoId}` : '/hub/arena',
      icon: <Swords className="w-5 h-5" />,
      matchPaths: ['/hub/arena/evento'],
    },
    {
      label: isAoVivo ? t('live') : t('nav_live'),
      href: '/hub/arena/live',
      icon: <Radio className="w-5 h-5" />,
      matchPaths: ['/hub/arena/live'],
      isLive: isAoVivo,
    },
    {
      label: t('nav_leagues'),
      ...
      matchPaths: ['/hub/arena/ligas'],
    },
    {
      label: t('nav_profile'),
      href: username ? `/hub/arena/perfil/${username}` : '/hub/arena/login',
      icon: <User className="w-5 h-5" />,
      matchPaths: ['/hub/arena/perfil'],
    },
  ];
