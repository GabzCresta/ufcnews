import type { DistribuicaoVitoriasSectionData } from '@/types/analise';
...
function MethodRow({
  ...
  const f1Wins = f1Pct > f2Pct;
  const f2Wins = f2Pct > f1Pct;
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right min-w-[60px]">
            <span className={`font-display text-2xl tabular-nums ${f1Wins ? 'text-white' : 'text-white/25'}`}>{f1Pct}%</span>
          </div>
          <div className="flex-1 h-2 rounded-full bg-white/[0.04] overflow-hidden">
            <div className={`h-full rounded-full ${f1Wins ? 'bg-ufc-red' : 'bg-ufc-red/20'}`} style={{ width: `${f1Pct}%` }} />
          </div>
          <span className={`text-xs tabular-nums ${f1Wins ? 'text-white/50' : 'text-white/20'}`}>{f1Count}</span>
        </div>
        ...
        <div className="flex items-center gap-3 flex-row-reverse">
          <div className="text-left min-w-[60px]">
            <span className={`font-display text-2xl tabular-nums ${f2Wins ? 'text-white' : 'text-white/25'}`}>{f2Pct}%</span>
          </div>
          <div className="flex-1 h-2 rounded-full bg-white/[0.04] overflow-hidden flex justify-end">
            <div className={`h-full rounded-full ${f2Wins ? 'bg-blue-400' : 'bg-blue-400/20'}`} style={{ width: `${f2Pct}%` }} />
          </div>
          <span className={`text-xs tabular-nums ${f2Wins ? 'text-white/50' : 'text-white/20'}`}>{f2Count}</span>
        </div>
      </div>
    </div>
  );
    ...
    <section>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-baseline mb-2">
        <div>
          ...
          <span className="ml-2 text-xs text-white/25">{f1.total_wins}W</span>
        </div>
        <div className="px-4" />
        <div className="text-right">
          <span className="font-display text-sm uppercase tracking-wider text-blue-400">{f2.nome}</span>
          <span className="ml-2 text-xs text-white/25">{f2.total_wins}W</span>
        </div>
      </div>
