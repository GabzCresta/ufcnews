...
function EventCard({
  ...
  const t = useTranslations('analises_hub');
  ...
  return (
    <Link href={`/analise/evento/${event.slug}`} className="group block">
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex-1">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-dark-textMuted">{event.evento_data}</span>
              <span className="text-xs text-dark-textMuted">· {event.evento_local}</span>
            </div>
            ...
            <div className="mt-2 flex items-center gap-2">
              <span className={`inline-flex items-center gap-1.5 rounded-full border border-current/20 px-3 py-1 text-xs font-bold ${badgeBg}`}>
                ...
                {event.total_fights} {t('lutas_analisadas')}
              </span>
            </div>
    ...
    <div className="min-h-screen bg-dark-bg">
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="mb-6 flex items-center gap-2 text-sm text-dark-textMuted">
          ...
          <span>/</span>
          <span className="text-dark-text">{t('breadcrumb')}</span>
        </div>

        <h1 className="mb-2 font-display text-3xl uppercase text-dark-text md:text-4xl">
          {t('title')} <span className="text-ufc-red">{t('title_accent')}</span>
        </h1>
        <p className="mb-8 text-dark-textMuted">{t('subtitle')}</p>
        ...
        {!hasAnySemanal && !hasAnyAnteriores && (
          <div className="rounded-lg border border-dark-border bg-dark-card p-12 text-center">
            ...
            <p className="mt-2 text-sm text-dark-textMuted">{t('nenhuma_desc')}</p>
          </div>
        )}
