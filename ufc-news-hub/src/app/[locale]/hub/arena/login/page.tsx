...
function LoginContent() {
  ...
  useEffect(() => {
    if (isAuthenticated && !authLoading) {
      router.push(redirectTo.replace('/arena', '/hub/arena'));
    }
      ...
      <div className="neu-card p-8 animate-glow-pulse-border">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
          ...
          <div className="text-center text-sm text-dark-textMuted slide-up-fade" style={{ animationDelay: '300ms' }}>
            {t('login_no_account')}{' '}
            <Link href={`/hub/arena/registro${redirectTo !== '/arena' ? `?redirect=${encodeURIComponent(redirectTo)}` : ''}`} className="text-ufc-red hover:text-ufc-redLight font-medium">
