...
export async function POST(request: Request): Promise<NextResponse<SyncResult>> {
  ...
  if (cronSecret) {
    const authHeader = (request as unknown as { headers: Headers }).headers.get('authorization');
    if (authHeader !== `Bearer ${cronSecret}`) {
      return NextResponse.json({ success: false, error: 'Unauthorized', adicionadas: 0, duplicadas: 0, rejeitadas: 0, processadas: 0 } as unknown as SyncResult, { status: 401 });
    }
  }
