# Fight Analyst - 15-Section Data Specification

Complete data contract for the `FullAnalysisData` object generated by the fight-analyst agent (used for every fight on a card, main card and prelims alike).

## Section 1: `hero` (HeroSectionData)
Component: `HeroSection` (no SectionHeader number, full-bleed layout)

```typescript
{
  evento_nome: string,           // 'UFC 326' or 'UFC Fight Night 270'
  evento_data: string,           // '7 de Marco, 2026' (Portuguese months, no accents)
  evento_local: string,          // 'T-Mobile Arena, Las Vegas'
  categoria_peso: string,        // 'Peso Leve' or 'Peso Pena (145 lbs)'
  num_rounds: number,            // typically 5 for main events
  titulo_em_jogo: string | null, // 'Titulo BMF', 'Titulo Peso Pena', or null
  tagline: string,               // dramatic one-liner: 'A Redencao de Um Round'
  tagline_sub: string,           // secondary context: '11 anos depois, a vinganca chega'
  fighter1: {
    nome_completo: string,       // 'Max "Blessed" Holloway' (can include nickname)
    apelido: string,             // 'Blessed' (nickname alone, or '' if none)
    sobrenome: string,           // 'Holloway' (used for display throughout all components)
    record: string,              // 'W-L-D' format: '27-8-0'
    ranking: string,             // '#4 LW' or '#1 Peso Pena' or 'Campeao'
    info_extra: string,          // 'Waianae, Hawaii | 34 anos'
    imagem_fullbody_url: string | null,  // UFC.com fullbody image URL or null
  },
  fighter2: { /* identical structure */ },
}
```

**Fighter Image URLs:** Found by WebFetching `https://www.ufc.com/athlete/[fighter-slug]` and extracting the `athlete_bio_full_body` image. Pattern: `https://www.ufc.com/images/styles/athlete_bio_full_body/s3/YYYY-MM/...png?itok=...`

**Date format:** Portuguese months without accents: Janeiro, Fevereiro, Marco, Abril, Maio, Junho, Julho, Agosto, Setembro, Outubro, Novembro, Dezembro.

## Section 2: `narrativa` (NarrativaSectionData)
Component: `NarrativaSection` (SectionHeader number="01", title="O Contexto Que Ninguem", accent="Te Conta")
Extra props: receives `fighter1Name` and `fighter2Name` (from hero.fighter1/2.sobrenome)

```typescript
{
  html_content: string,          // HTML prose with the fight narrative
  stakes: StakeRow[],            // can be empty [] or populated
  prognostico?: {                // optional but SHOULD always be included
    fighter1_vence: FuturoCenario,
    fighter2_vence: FuturoCenario,
  },
}
```

**`html_content` rules:**
- Use only: `<h3>`, `<p>`, `<strong>`, `<em>`
- Fighter1 name highlights: `<strong class="text-ufc-red">Holloway</strong>`
- Fighter2 name highlights: `<strong class="text-blue-400">Oliveira</strong>`
- Section headers inside HTML: `<h3 class="font-display text-xl uppercase text-ufc-red mb-4">Titulo</h3>`
- Subsequent headers add: `mt-8` class
- Write as if telling a story to a friend who follows UFC

**`stakes` (StakeRow):**
```typescript
{ dimensao: string, fighter1: string, fighter2: string }
```
Typical dimensoes: 'Ranking', 'Objetivo', 'Narrativa', 'Risco', 'Recompensa', 'Title Shot', 'Legado'

**`prognostico` (FuturoCenario):**
```typescript
{
  titulo: string,          // ALL CAPS dramatic headline: 'A COROACAO DO BLESSED'
  subtitulo: string,       // one-liner summary
  consequencias: [         // 3-4 consequences
    { tag: string, texto: string },  // tag: short ALL CAPS category
  ],
  proxima_luta: string,    // what fight comes next if this fighter wins
}
```

## Section 3: `momento_atual` (MomentoAtualSectionData)
Component: `MomentoAtualSection` (SectionHeader number="02", title="Momento", accent="Atual")
Uses: `'use client'` (has modal for full fight history)

```typescript
{
  fighter1: MomentoAtualFighter,
  fighter2: MomentoAtualFighter,
}
```

**MomentoAtualFighter:**
```typescript
{
  nome: string,                // display name: 'Max Holloway'
  color: 'red' | 'blue',      // fighter1 = 'red', fighter2 = 'blue'
  recent_fights: RecentFight[],       // 4-5 most recent fights
  full_fight_history?: RecentFight[], // complete UFC career (for modal), oldest first
  layoff_warning?: string | null,     // null if active, warning text if >12 months inactive
  momentum_score: number,     // 0-10 (NOT rendered by component but keep for data)
  momentum_label: string,     // 'Em Alta', 'Em Ascensao', 'Em Recuperacao', 'Estavel (com ressalvas)'
  momentum_trend: 'ascending' | 'descending' | 'stable' | 'resilient',
  momentum_note: string,      // paragraph explaining momentum assessment
}
```

**RecentFight:**
```typescript
{
  date: string,              // 'Abr 2023' (abbreviated Portuguese month + year)
  opponent: string,          // 'Arnold Allen'
  result: 'W' | 'L' | 'D' | 'NC',
  method: string,            // 'Decisao Unanime', 'KO R5', 'Sub R2', 'TKO R4'
  opponent_rank: string,     // '#6 FW', '#3 LW', 'Campeao', 'N/R'
  quality_score: number,     // 1-5 - CALIBRE DO OPONENTE (see table below)
  quality_label?: string,    // 'Ruim'|'Medio'|'Bom'|'Muito Bom'|'Excelente'
  note: string,              // brief context: 'Dominou 5 rounds inteiros'
}
```

**REGRA CRITICA: `quality_score` mede o CALIBRE DO OPONENTE, NAO a performance do lutador.**
O score e sobre QUEM ele enfrentou, nao como foi a luta.

| Calibre do Oponente | Score | Label |
|---------------------|-------|-------|
| Campeao, ex-campeao recente, top 3 | 5 | Excelente |
| Top 5-7, contender estabelecido | 4 | Muito Bom |
| Top 8-15, prospect promissor ranqueado | 3 | Bom |
| Sem ranking mas UFC veteran, gatekeeper | 2 | Medio |
| Sem ranking, inicio de carreira UFC, estreante | 1 | Ruim |

**Date abbreviations:** Jan, Fev, Mar, Abr, Mai, Jun, Jul, Ago, Set, Out, Nov, Dez

**Method formats:**
- recent_fights: 'Decisao Unanime', 'KO R5', 'Sub R2', 'TKO R3 (socos)', 'Decisao Dividida'
- full_fight_history: Can be abbreviated: 'UD', 'SD', 'KO R1', 'TKO R2', 'Sub R1'

## Section 4: `nivel_competicao` (NivelCompeticaoSectionData)
Component: `NivelCompeticaoSection` (SectionHeader number="03", title="Nivel de", accent="Competicao")

```typescript
{
  fighter1: {
    nome: string,
    media_oponentes: number,           // 1-5 scale
    media_oponentes_label?: string,    // 'Ruim'|'Medio'|'Bom'|'Muito Bom'|'Excelente'
    aproveitamento: string,            // 'NW-NL (percent%)' format: '4W-1L (80%)'
    contra_top5: string,               // 'NW-NL' format: '2W-1L'
  },
  fighter2: { /* same */ },
  oponentes_em_comum_count: { fighter1: number, fighter2: number },
  oponentes_em_comum_note: string,
}
```

## Section 5: `oponente_comum` (OponenteComumSectionData | null)
Component: `OponenteComumSection` (SectionHeader number="04", title="Spotlight:", accent=DYNAMIC opponent name)
**OPTIONAL:** Set to `null` if no meaningful common opponent exists.

```typescript
{
  oponente_nome: string,         // 'Dan Ige'
  fighter1_result: {
    resultado: string,           // 'Vitoria por Decisao Unanime' or 'Derrota'
    metodo: string,              // 'KO R3 (1:34)' or scorecards
    duracao: string,             // '2 rounds completos + 1:34' or '3 rounds (15:00)'
    contexto: string,            // paragraph: HOW did the fight play out
    performance: string,         // paragraph: assessment
    evento: string,              // 'UFC 308'
    data: string,                // 'Out 2024'
  },
  fighter2_result: { /* same structure */ },
  insight: string,               // comparative paragraph
}
```

## Section 6: `comparacao_estatistica` (ComparacaoEstatisticaSectionData)
Component: `ComparacaoEstatisticaSection` (SectionHeader number="05", title="Comparacao", accent="Estatistica")
Extra props: receives `fighter1Name` and `fighter2Name`

```typescript
{
  stats: StatBarData[],          // 7-8 stat comparisons
  tale_of_tape: TaleOfTapeRow[], // 5-6 physical comparisons
}
```

**StatBarData:**
```typescript
{
  label: string,             // 'Sig. Strikes por Minuto'
  valueA: number,            // fighter1 value
  valueB: number,            // fighter2 value
  maxVal: number,            // max for bar scale
  format?: 'decimal' | 'percent' | 'integer',
  note?: string,
  reverseWinner?: boolean,   // true when LOWER is better (e.g., Strikes Absorbed)
}
```

**Standard stat labels:**
1. `'Sig. Strikes por Minuto'` (decimal, maxVal: 6-8)
2. `'Precisao de Strikes (%)'` (percent, maxVal: 100)
3. `'Strikes Absorvidos/Min'` (decimal, maxVal: 6, reverseWinner: true)
4. `'Defesa de Strikes (%)'` (percent, maxVal: 100)
5. `'Takedowns por 15 Min'` (decimal, maxVal: 5)
6. `'Precisao de Takedown (%)'` (percent, maxVal: 100)
7. `'Defesa de Takedown (%)'` (percent, maxVal: 100)
8. `'Submissoes por 15 Min'` (decimal, maxVal: 3) - optional

**TaleOfTapeRow:**
```typescript
{
  label: string,             // 'Idade', 'Altura', 'Envergadura', 'Stance', 'Academia'
  fighter1: string,
  fighter2: string,
  note?: string | null,
}
```

## Section 7: `perfil_habilidades` (PerfilHabilidadesSectionData)
Component: `PerfilHabilidadesSection` (SectionHeader number="06", title="Perfil de", accent="Habilidades")
Extra props: receives `fighter1Name` and `fighter2Name`

```typescript
{
  skills: SkillBarData[],    // exactly 6 skills
  insight?: string,          // optional matchup insight paragraph
}
```

**SkillBarData:**
```typescript
{
  label: string,                   // tailored per matchup
  valueA: number,                  // 0-100
  valueB: number,                  // 0-100
  labelA?: string,                 // 'Excelente'|'Muito Bom'|'Bom'|'Medio'|'Ruim'
  labelB?: string,
  advantage?: 'fighter1' | 'fighter2' | 'even',
  advantage_note?: string,
}
```

**Value-to-label mapping:** 90-100=Excelente, 75-89=Muito Bom, 55-74=Bom, 35-54=Medio, 0-34=Ruim

## Section 8: `distribuicao_vitorias` (DistribuicaoVitoriasSectionData)
Component: `DistribuicaoVitoriasSection` (SectionHeader number="07", title="Distribuicao de", accent="Vitorias")

```typescript
{
  fighter1: {
    nome: string,
    ko_tko: { count: number, percent: number },
    submission: { count: number, percent: number },
    decision: { count: number, percent: number },
    total_wins: number,
  },
  fighter2: { /* same */ },
  insight: string,
}
```

**MATH CHECK:** ko_tko.count + submission.count + decision.count = total_wins. Percentages sum to ~100.

## Section 8b: `distribuicao_derrotas` (DistribuicaoDerrotasSectionData) — MANDATORY (added 2026-05-05)
Component: `DistribuicaoDerrotasSection` (SectionHeader number="08", title="Distribuicao de", accent="Derrotas")

Mirror of `distribuicao_vitorias`, but for losses. **Equally critical for value bets** — knowing HOW a fighter loses (KO-prone vs sub-prone vs grinds out decisions) is the missing dimension that pure W-L records hide. Tapology shows this in the fighter header (e.g. tapology.com/fightcenter/fighters/92011-clayton-carpenter).

```typescript
{
  fighter1: {
    nome: string,
    ko_tko: { count: number, percent: number },
    submission: { count: number, percent: number },
    decision: { count: number, percent: number },
    total_losses: number,
  },
  fighter2: { /* same */ },
  insight: string,  // comparative paragraph: vulnerability pattern, sample size caveats, how this informs method-pick
}
```

**MATH CHECK:** ko_tko.count + submission.count + decision.count = total_losses. Percentages sum to ~100.

**Source priority for loss breakdown:**
1. **Tapology fighter page** (primary — header shows method splits explicitly)
2. **UFCStats fighter page** (filter by losses)
3. **Sherdog fight log**
4. NEVER Wikipedia

**Insight rules:**
- Comparative, not descriptive. "Holloway perdeu 6 de 8 em decisão (queixo durável); Topuria perdeu 1 de 1 por sub (amostra pequena, mas BJJ é o gap historico)" — NOT "Holloway tem X derrotas".
- Flag small samples (<3 losses) explicitly. A fighter at 1-1 with 1 loss by KO doesn't have a "KO vulnerability pattern".
- Connect to the matchup. If fighter1 has 3/4 losses by sub and fighter2 is a BJJ specialist, say so.

**If `total_losses` is 0:** omit the section entirely from that fighter's render (the field is optional). The component will not render if `distribuicao_derrotas` is absent.

**Estimation forbidden** — same protocol as `distribuicao_vitorias`. Cross-reference at least 2 sources. If unverifiable, leave the section out rather than guess.

## Section 9: `danger_zones` (DangerZonesSectionData)
Component: `DangerZonesSection` (SectionHeader number="08", title="Danger", accent="Zones")

```typescript
{
  zones: DangerZoneCard[],   // exactly 3 zone cards
}
```

**DangerZoneCard:**
```typescript
{
  rounds: string,            // 'R1', 'R2-R3', 'R4-R5'
  danger_level: number,      // 1-10 (NOT rendered)
  danger_label: string,      // 'VANTAGEM EVLOEV', 'EQUILIBRADO', 'ROUND DECISIVO'
  color: 'red' | 'gold' | 'green',
  title: string,             // 'Territorio do Oliveira'
  description: string,       // paragraph
}
```

**NEVER use** `'PERIGO 7/10'` or numeric scales in the label. Use advantage-based labels.

**Color:** `'red'` = strong advantage, `'gold'` = balanced/pivotal, `'green'` = advantage other fighter

## Section 10: `intangiveis` (IntangiveisSectionData)
Component: `IntangiveisSection` (SectionHeader number="09", title="Fatores Invisiveis &", accent="Red Flags")

```typescript
{
  items: IntangivelItem[],   // 5-7 items
}
```

**IntangivelItem:**
```typescript
{
  icon: string,              // Lucide icon name
  title: string,             // 'Mao direita lesionada'
  fighter: string,           // fighter display name: 'Holloway'
  risk_level: string,        // 'RISCO ALTO'|'RISCO MEDIO'|'RISCO BAIXO'|'POSITIVO'|'ENORME POSITIVO'|'NEUTRO'
  risk_color: 'red' | 'yellow' | 'green' | 'neutral',
  description: string,
}
```

**risk_level to risk_color mapping:**
- 'RISCO ALTO' → 'red'
- 'RISCO MEDIO' → 'yellow'
- 'RISCO BAIXO' → 'yellow' or 'green'
- 'POSITIVO' / 'ENORME POSITIVO' → 'green'
- 'NEUTRO' → 'neutral'

**Icons:** AlertTriangle (injuries), Clock (layoffs), TrendingUp (momentum), Zap (energy/KO power), Brain (psychological), MapPin (venue), Shield (durability), Activity (cardio), Target (vulnerabilities)

## Section 11: `caminhos_vitoria` (CaminhosVitoriaSectionData)
Component: `CaminhosVitoriaSection` (SectionHeader number="10", title="Caminhos Para", accent="Vitoria")

```typescript
{
  fighter1: {
    nome: string,
    total_probability: number,       // 0-100
    scenarios: CaminhoVitoria[],     // 3-4 scenarios
  },
  fighter2: {
    nome: string,
    total_probability: number,
    scenarios: CaminhoVitoria[],
  },
}
```

**CaminhoVitoria:**
```typescript
{
  name: string,              // dramatic name: 'Morte por Mil Cortes'
  probability: number,
  method: string,            // 'TKO R4-5 ou UD'
  description: string,
}
```

**MATH RULE:** fighter1.total_probability + fighter2.total_probability + ~3 (draw) must equal ~100.

## Section 12: `previsao_final` (PrevisaoFinalSectionData)
Component: `PrevisaoFinalSection` (SectionHeader number="11", title="Previsao", accent="Final")

```typescript
{
  winner_name: string,
  winner_side: 'fighter1' | 'fighter2',
  predicted_method: string,          // 'TKO tardio ou Decisao'
  confidence_score: number,          // 1-10
  confidence_label: string,          // 'MEDIA', 'MEDIA-ALTA', 'ALTA'
  explanation: string,
  x_factor: { title: string, description: string },
  upset_alert: { title: string, description: string },
  probabilities: {
    fighter1: { nome: string, percent: number },
    fighter2: { nome: string, percent: number },
    draw: number,                    // typically 2-3
  },
  value_picks?: {
    moneyline: { pick: string, reasoning: string },
    method: { pick: string, reasoning: string },
    over_under: { pick: string, rounds: number, reasoning: string },
    best_value: string,
  },
}
```

**CONFIDENCE:** >= 8 only if favorite >65% AND method is broad. Most fights = MEDIA (5-6) or MEDIA-ALTA (7).

### NEW: `conviction` sub-object (MANDATORY for main card from 2026-04-06)

```typescript
conviction: {
  thesis: string,                  // 3-sentence thesis in mold:
                                   // "A tese e: [WINNER] vence porque [r1 + source], [r2 + source], e [r3 + source].
                                   //  O caminho pra isso e [round-by-round]. Isso desmorona se [condition]."
  conviction_score: number,        // 1-10. CAPPED AT 5 if rationale is not specific beyond stats/odds.
  conviction_rationale: string,    // WHY this score — must cite something specific
  conviction_breakers: string[],   // 2-4 specific conditions that invalidate the thesis
  underdog_path: {
    viable: boolean,
    probability_estimate: number,  // must match loser's probabilities.percent (±3)
    key_scenario: string,          // step-by-step of HOW the underdog wins
    required_conditions: string[], // 2-4 specific requirements
    historical_precedent?: string, // optional, strongly preferred
  },
}
```

**Conviction rules:**
- 9-10 = rare, 4+ convergent qualitative evidences + historical precedent
- 7-8 = solid thesis, 2-3 strong evidences, plausible breakers
- 5-6 = default, "close fight but I see the path" — `explanation` MUST say so explicitly
- 3-4 = honest coinflip, must admit it
- 1-2 = do not pick, write "tossup"
- `conviction_score` is INDEPENDENT from `confidence_score`. Confidence = probability math. Conviction = thesis defense.
- `underdog_path` is MANDATORY even at conviction 9-10. Never leave `key_scenario` vague.

**Prelims `conviction`:** Same object, but `thesis` is 1 sentence, `conviction_breakers` may be `[]`, `historical_precedent` may be omitted.

## Section 13: `o_que_observar` (OQueObservarSectionData)
Component: `OQueObservarSection` (SectionHeader number="12", title="O Que", accent="Observar")

```typescript
{
  points: TalkingPoint[],   // exactly 5 points
}
```

**TalkingPoint:**
```typescript
{
  num: number,               // sequential 1-5
  title: string,
  icon: string,              // Lucide icon name
  description: string,       // 3-4 line paragraph
}
```

## Section 14: `creator_kit` (CreatorKitSectionData)
Component: `CreatorKitSection` (SectionHeader number="13", title="Creator", accent="Kit")
Uses: `'use client'` (has tabs and clipboard copy)

```typescript
{
  instagram: InstagramSlide[],     // 3-5 slides
  twitter: Tweet[],                // 5-6 tweets
  video: VideoScriptSection[],     // 4-5 sections totaling ~60s
  tiktok?: TikTokScript[],        // 3 scripts
  headlines?: string[],            // 5-6 headlines
}
```

**InstagramSlide:** `{ slide_number, title, content (\\n for line breaks), color: 'red'|'blue'|'gold' }`

**Tweet:** `{ num: 'N/TOTAL', text }`

**VideoScriptSection:** `{ time: '0-10s', title: 'Hook', text }`

**TikTokScript:** `{ hook, body, cta }`

## Section 15: `radar_apostador` (RadarApostadorSectionData)
Component: `RadarApostadorSection` (SectionHeader number="15", title="Radar do", accent="Apostador")
Uses: `'use client'` (expandable value pick cards)

```typescript
{
  odds: {
    fighter1_odds: string,     // American odds: '-230'
    fighter2_odds: string,     // '+185'
    fighter1_name: string,
    fighter2_name: string,
    source: string,            // 'Media de DraftKings, FanDuel e BetMGM (marco 2026)'
  },
  edges: EstatisticoEdge[],    // 5-6 statistical edges
  value_picks: ValuePick[],    // 3-4 value picks
  armadilha: {
    titulo: string,            // 'Armadilha: Oliveira por Submission'
    descricao: string,
  },
  disclaimer: string,          // 'Analise estatistica para fins informativos. Aposte com responsabilidade.'
}
```

**EstatisticoEdge:**
```typescript
{
  icon: string,
  titulo: string,
  stat_headline: string,     // ALL CAPS: '75+ SIG STRIKES EM 17/17 LUTAS DESDE 2016'
  contexto: string,
  implicacao_aposta: string,
  edge_level: 'forte' | 'moderado' | 'leve',
  fighter_side?: 'fighter1' | 'fighter2' | 'neutral',
}
```

**ValuePick:**
```typescript
{
  tipo: string,              // 'Over/Under', 'Duracao', 'Metodo', 'Moneyline'
  pick: string,
  odds: string,
  confianca: 'baixa' | 'media' | 'alta',
  edge_vs_mercado?: string,
  raciocinio: string,
}
```

### Evidence Rules for `radar_apostador` (NEW — 2026-04-06)

- Every `EstatisticoEdge.stat_headline` must be a number traceable to a source (UFCStats URL, specific fight, outlet). "Recent trend" is banned.
- `EstatisticoEdge.contexto` must name the fights or time window the stat comes from.
- `ValuePick.raciocinio` follows a mini-thesis mold: "[Pick] porque [evidence + source]. Mercado errado porque [reason]. Desmorona se [condition]."
- **Coherence rule with main conviction:** If `previsao_final.conviction.conviction_score <= 6`, no `ValuePick` may have `confianca: 'alta'`, and `value_picks.moneyline` in `previsao_final` must not push the favorite aggressively. Internal contradiction is a blocking bug.
- `armadilha` must name a specific popular market pick and explain with evidence why it's a trap.
- Minimum: 3 sourced edges + 2 sourced value picks, or shrink the section.

### `betting_value` field
Always set to `null`. All betting content goes in `radar_apostador` and `previsao_final.value_picks`.
